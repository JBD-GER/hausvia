...
export default async function AdminInvoicesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <>
      <PageHeader
        eyebrow="Rechnungen"
        title="Rechnungen erstellen und senden"
        text="Rechnungen können aus Angeboten oder manuell erstellt, als PDF geladen und per E-Mail ins Kundenportal gesendet werden."
        actions={(
          <>
            <PortalDialog
              triggerLabel="Aus Angebot"
              triggerClassName={secondaryButtonClass}
              title="Rechnung aus Angebot erstellen"
              description="Übernehmen Sie Positionen und Kundendaten aus einem bestehenden Angebot."
            >
              <form action={createInvoiceFromOfferAction} className="grid gap-4 sm:grid-cols-2">
                <Field label="Angebot übernehmen" className="sm:col-span-2">
                ...
                <Field label="Leistungsmonat ab"><input name="servicePeriodStart" type="date" className={inputClass} /></Field>
                <Field label="Fällig optional"><input name="dueDate" type="date" className={inputClass} /></Field>
                <button className={`${buttonClass} sm:col-span-2`}>Als Entwurf anlegen</button>
              </form>
            </PortalDialog>
            <PortalDialog
              triggerLabel="Neue Rechnung"
              title="Neue Rechnung erstellen"
              description="Erstellen Sie einen vollständigen Rechnungsentwurf mit Positionen und Zahlungsangaben."
              size="wide"
            >
              <DocumentEditor
                kind="invoice"
      ...
      <div className="grid gap-5">
        <CompactSection
          title="Rechnungen suchen und filtern"
          description="Suche, Status und Sortierung"
          badge={(search || statusFilter || sort !== "newest") ? <StatusPill tone="info">Filter aktiv</StatusPill> : null}
        >
