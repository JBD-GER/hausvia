-- Use the real wall clock after the property advisory lock has been acquired.
-- PostgreSQL's now() is fixed at transaction start and can therefore become
-- stale while waiting for another planner transaction to finish.
...
begin
  ...
  where visit.property_id = v_damage.property_id
    ...
    and visit.scheduled_start > greatest(clock_timestamp(), v_damage.created_at)
    and (
